...
import { loadState, saveState, upsertBusiness, removeBusiness } from "../lib/state.js";
  ...
  server.tool(
    ...
    "Record which GTA Online businesses you own and their current state (resupplied just now, sold just now, stock level, upgrades). Persists locally so the server tracks fill timers and cooldowns across sessions.",
    {
      businesses: z
        .array(
          z.object({
            id: z.string().describe("Business id, e.g. acid-lab, bunker, cocaine, nightclub-warehouse"),
            resupply: z.boolean().optional().describe("Mark resupplied/production started now"),
            sold: z.boolean().optional().describe("Mark sold now (resets stock to empty)"),
            stockFraction: z.number().min(0).max(1).optional().describe("Current stock as 0..1"),
            lastResupplyAt: z.string().optional().describe("ISO time override for last resupply"),
            upgraded: z.boolean().optional().describe("Has the production/equipment upgrade"),
            notes: z.string().optional(),
          }),
        )
        .optional(),
      remove: z.array(z.string()).optional().describe("Business ids to stop tracking"),
    },
    async ({ businesses, remove }) => {
      ...
      for (const b of businesses ?? []) {
        ...
        if (typeof b.stockFraction === "number") patch.stockFraction = b.stockFraction;
      ...
      const lines = [
        ...
        unknown.length ? `Ignored unknown ids: ${unknown.join(", ")} (see gta-best-methods for valid ids)` : null,
  ...
  server.tool(
    ...
    async () => {
      ...
      for (const ob of owned) {
        ...
        if (!ref) continue;
        const fill = currentFill(ob.stockFraction ?? 0, ob.lastResupplyAt, ref.fillMinutes);
        ...
        const sellFlag = overSoloLimit ? " ⚠️ exceeds solo 1-vehicle limit — sell before it overfills" : "";
        rows.push(
