    ...
    "gta-business-economics",
    {
      ...
      inputSchema: {
        business: z
          ...
          .optional()
          .describe(
            "One business key (acid, bunk, coke, meth, cash, weed, docs); omit for all",
          ),
      },
    },
    async ({ business }) => {
      ...
      if (!econs.length) {
        return errorText(
          `Unknown business. Valid keys: ${Object.keys(BUSINESS_KEYS).join(", ")}`,
        );
      }
      const lines = econs.map((e) => {
        const far = Math.round(
          e!.fullValueLocal * (1 + e!.maxHighDemandPct / 100),
        );
        return (
      ...
      const footer = wk
        ...
        : "";
      return text(
        `Current business economics (gtaweb toolkit snapshot):\n\n${lines.join("\n")}${footer}`,
      );
    },
  );
}
