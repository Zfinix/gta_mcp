...
export function registerMapTools(server: McpServer): void {
  server.tool(
    ...
    "List or search every GTA Online map category available on GTALens (street dealers, gun van, exotic exports, drug vehicle, collectibles, business missions, Cayo, properties, and ~100 more). Filter by group, keyword, or money-only.",
    {
      group: z
        ...
        .optional(),
      search: z.string().optional().describe("Keyword to filter category names/slugs"),
      moneyOnly: z.boolean().optional().describe("Only money-relevant categories"),
    },
    async ({ group, search, moneyOnly }) => {
      ...
      if (search) {
        ...
        cats = cats.filter((c) => c.name.toLowerCase().includes(q) || c.slug.includes(q));
      }
      if (!cats.length) return text("No categories match. Try gta-map-categories with no filters to see all.");

      const byGroup = new Map<string, string[]>();
      for (const c of cats) {
        const tag = [c.money ? "💰" : "", c.daily ? "🔄daily" : ""].filter(Boolean).join(" ");
        byGroup.set(c.group, [...(byGroup.get(c.group) ?? []), `  • ${c.name} (${c.slug})${tag ? " " + tag : ""}`]);
      }
      const blocks = [...byGroup.entries()].map(([g, items]) => `${g}:\n${items.join("\n")}`);
      return text(
  ...
  server.tool(
    ...
    {
      category: z.string().describe("Category slug or name (see gta-map-categories)"),
      includeLiveData: z.boolean().default(true).describe("Pull today's live panel text from the map"),
    },
    async ({ category, includeLiveData }) => {
      const cat = findMapCategory(category);
      if (!cat) return text(`No map category matching "${category}". Use gta-map-categories to list them.`);
  ...
  server.tool(
    ...
    async ({ category }) => {
      ...
      try {
        ...
        return result([
          { type: "text", text: `GTALens map — ${cat.name} (${gtalensMapUrl(cat.slug)})` },
  ...
  registerQuick(server, "gta-street-dealers", "street-dealers", "Today's GTA Online street drug dealers: map + the live locations and which products sell highest there.");
  registerQuick(server, "gta-exotic-exports", "exotic-exports", "Today's GTA Online Exotic Exports special-vehicle wanted list (the 10 cars to source for the Auto Shop) + map.");
  registerQuick(server, "gta-gun-van", "gun-vans", "Today's GTA Online Gun Van location + map (daily discounted weapons/armor/MkII ammo).");
  registerQuick(server, "gta-drug-vehicle", "drug-vehicle", "Today's GTA Online freemode Drug Vehicle location (sells for a bonus) + map.");
}

function registerQuick(server: McpServer, name: string, slug: string, desc: string): void {
  server.tool(name, desc, {}, async () => {
    ...
    const content: Content[] = [{ type: "text", text: `Map: ${url}\n\n${panel}` }];
    if (shot) content.push({ type: "image", data: shot.imageBase64, mimeType: shot.mimeType });
    return { content };
  });
}
