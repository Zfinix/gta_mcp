    ...
    "gta-money-plan",
    {
      ...
      inputSchema: {
        sessionMinutes: z
          ...
          .max(600)
          .default(60)
          .describe("How long you can play right now"),
        solo: z.boolean().default(true),
        goal: z
          .enum(["max-cash", "fast-cash", "collectibles", "balanced"])
          .default("balanced"),
      },
    },
    async ({ sessionMinutes, solo, goal }) => {
      ...
      try {
        ...
        const res = await withCache(
          "newswire-weekly",
          currentWeeklyWindowKey(now),
          ttl,
          getWeeklyUpdate,
        );
        weeklyTitle = res.value.title;
        weeklyText =
          `${res.value.title} ${res.value.bonuses.join(" ")}`.toLowerCase();
      ...
      kickoff.push(
        "Activate the Mansion production boost on today's sell target (rotate daily) — if you own one.",
      );
      ...
      if (empties.length) {
        kickoff.push(
          `Resupply: ${empties.map((e) => e.ref!.name).join(", ")} — start them producing before you do anything else.`,
        );
      } else if (!tracked.length) {
        kickoff.push(
          "Track your businesses with gta-business-set-state so this plan can manage resupply/sell timing.",
        );
      }
      kickoff.push(
        "Set Nightclub technicians to Coke/Meth/Cash and top up popularity.",
      );
      ...
      for (const a of actions) {
        ...
        if (goal === "fast-cash")
          s = perMin(a) * (a.estMinutes <= 20 ? 1.5 : 1);
        if (goal === "collectibles")
          s = a.title.includes("collectibles") ? 1e9 : perMin(a);
      ...
      const urgencyTag = (u: Action["urgency"]) =>
        u === "now" ? "🔴 NOW" : u === "high" ? "🟠" : "";

      const planLines = plan.map(
        (a, i) =>
          `${i + 1}. ${urgencyTag(a.urgency)} ${a.title} — ${a.why}\n   est. ${money(a.estPayout)} in ~${a.estMinutes} min`,
      );
      ...
      if (weeklyTitle)
        ...
        footerBits.push(
          "⚠️ Could not load this week's bonuses — values not bonus-weighted. Check gta-weekly-bonuses.",
        );
